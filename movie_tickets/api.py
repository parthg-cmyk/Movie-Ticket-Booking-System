...
def get_booked_seats(show: str) -> list:
	...
	bookings = frappe.get_all("Ticket Booking", filters={"show": show, "docstatus": 1}, pluck="name")
	...
	return [f"{s.row_letter}-{s.seat_number}" for s in seats if s.row_letter and s.seat_number]
...
def get_shows_for_movie(movie: str, city: Optional[str] = None, date: Optional[str] = None) -> List[Dict]:
	...
	if city:
		shows = [s for s in shows if frappe.db.get_value("Theater", s["theater"], "city") == city]
...
def process_bulk_shows(movie: str, screens: List[str], from_date: str, to_date: str, show_times: List[str]):
	"""
	...
	# ✅ Logging
	frappe.logger().info(f"Bulk Show Creation Completed | Created: {total_created}, Skipped: {total_skipped}")
